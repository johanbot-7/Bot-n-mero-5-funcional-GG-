...
Intentó acceder sin autorización 3 veces."""
    ...
    # 🔹 BOTÓN
    botones = [
        [InlineKeyboardButton("👥 Grupo Oficial➡️", url="https://t.me/+RrL593ltDKgwMDJh")]
    ]
...
async def mantenimiento_on(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    guardar_json(MANTENIMIENTO_FILE, modo_mantenimiento)

    await update.message.reply_text("🛠 Modo mantenimiento ACTIVADO")

    # enviar a usuarios permitidos
    for uid in usuarios:
        try:
            await context.bot.send_message(
                chat_id=int(uid),
                text="🔧 El bot está en mantenimiento temporal. Por favor intenta más tarde."
            )
        except:
            pass
async def mantenimiento_off(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✅ Modo mantenimiento DESACTIVADO")

    # avisar a usuarios permitidos
    for uid in usuarios:
        try:
            await context.bot.send_message(
                chat_id=int(uid),
                text="✅ ¡El bot está activo nuevamente! Ahora puedes usar funciones con /start."
            )
        except:
            pass
...
app.add_handler(CommandHandler("mantenimiento_on", mantenimiento_on))
app.add_handler(CommandHandler("mantenimiento_off", mantenimiento_off))
